...
<div class="flex-1 flex mt-20 items-center justify-between ">
    ...
    <a href="/admin/manage-yojna/insert" class="bg-indigo-500 hover:bg-indigo-800 text-white px-3 py-2 rounded">
        <i class="fas fa-plus"></i>Add New Yojna</a>
</div>
        ...
        <table class="min-w-full bg-white border border-gray-200" id="callingPlan">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="border-b border-gray-200 px-3 py-2 text-sm">Hindi Name</th>
                    <th class="border-b border-gray-200 px-3 py-2 text-sm">English Name</th>
            ...
            let callingPlans = () => {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        data.forEach((data) => {
                            table.append(`
                                <tr class="mt-5 text-center">
                                    <td class="border-b border-gray-200 px-3  py-2 text-sm">${data.id}</td> 
                                    <td class="border-b border-gray-200 px-3  py-2 text-sm">${data.hname}</td> 
                                    <td class="border-b border-gray-200 px-3  py-2 text-sm">${data.ename}</td> 
                                    <td class="border-b border-gray-200 px-3  py-2 text-sm">Rs.${data.fees}</td>
                                    <td class="border-b border-gray-200 px-3  flex justify-center py-2 text-sm">
                                        <a href='/admin/manage-yojna/${data.id}'
...
@section('content')
    ...
    <div class="flex justify-end mt-4">
        <button onclick="let printContents = document.getElementById('callingPlan').outerHTML; let originalContents = document.body.innerHTML; document.body.innerHTML = printContents; window.print(); document.body.innerHTML = originalContents;" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            Print This Page
        </button>
    </div>
                    ...
                    success: function(response) {
                        ...
                        data.forEach((data, index) => {
                            ...
                            <tr class="mt-5 text-center">
                                ...
                                <td class="border-b border-gray-200 px-3 py-2 text-sm">Rs.${data.fees}</td>
                                <td class="border-b border-gray-200 px-3 flex justify-center py-2 text-sm">
                                    <a href='/admin/manage-yojna/${data.id}' class="py-1 px-2">
