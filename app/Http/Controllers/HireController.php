...
class HireController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'mobile' => 'required|digits:10|unique:users|regex:/^[0-9]{10}$/',
            'plan_id' => 'required',
        ...
        if ($hire) {
            ...
            if ($user) {
                ...
                try {
                    Mail::send('emails.account_created', ['user' => $user], function ($message) use ($request) {
                        $message->to($request->email)->subject('Company Added To Taskinn Solutions');
                    });

                    return response()->json(['success' => true, 'message' => 'Company added successfully.']);
                } catch (\Exception $e) {
                    return response()->json(['success' => false, 'message' => 'Failed to add company.']);
                }
            }
